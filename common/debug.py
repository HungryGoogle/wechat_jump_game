...
def backup_screenshot(timeStamp):
    """
    为了方便失败的时候 debug
    """
    make_debug_dir(screenshot_backup_dir)
    shutil.copy('autojump.png', '{}{}.png'.format(screenshot_backup_dir, timeStamp))


def save_debug_screenshot(timeStamp, image, piece_x, piece_y, board_x, board_y):
    """
    对 debug 图片加上详细的注释
    """
    make_debug_dir(screenshot_backup_dir)
    ...
    # 出发点
    draw.line((piece_x, 0, piece_x, image.size[1]), fill=(255, 0, 0))
    draw.line((0, piece_y, image.size[0], piece_y), fill=(255, 0, 0))
    ...
    del draw
    image.save('{}{}_d.png'.format(screenshot_backup_dir, timeStamp))
